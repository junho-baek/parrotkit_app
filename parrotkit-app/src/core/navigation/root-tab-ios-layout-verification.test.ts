...
const iosViewports = rootBottomNavQaViewports.filter((viewport) => viewport.platform === 'ios');

if (iosViewports.length < 2) {
...
for (const viewport of iosViewports) {
  ...
  if (layout.height < rootTabBarTopPadding + rootTabMinimumTouchTarget + layout.paddingBottom) {
    throw new Error(`${viewport.id} must reserve vertical room for tap targets plus bottom safe-area padding.`);
  }

  if (layout.paddingBottom !== Math.max(viewport.bottomInset, rootTabBarMinBottomPadding)) {
    throw new Error(`${viewport.id} must preserve the iOS home-indicator safe-area padding exactly once.`);
  }

  if (layout.height !== rootTabBarContentHeight + layout.paddingBottom) {
    throw new Error(`${viewport.id} tab bar height must be content height plus bottom safe-area padding.`);
  }
  ...
  if (rootTabBarCenterActionDiameter < rootTabMinimumTouchTarget * 1.25) {
    throw new Error(`${viewport.id} Paste action must be visibly larger than the neighboring tab targets.`);
  }
