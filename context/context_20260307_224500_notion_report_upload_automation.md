...
## Deployment Automation Extension
- `scripts/generate-deployment-report.cjs`를 추가해 Vercel/GitHub deployment metadata로 Markdown/PDF 보고서를 자동 생성하게 했다.
- `scripts/init-report-summary.cjs`를 추가해 PDF/PPT 산출물용 summary markdown template을 만들 수 있게 했다.
- `.github/workflows/vercel-notion-auto-report.yml`를 추가해 `vercel.deployment.ready` 이벤트 후 자동으로 deployment report를 생성하고 Notion에 업로드하게 했다.
- GitHub repo `junho-baek/Parrotkit-deploy`에 `NOTION_API_KEY`, `NOTION_REPORTS_DATA_SOURCE_ID` secrets를 설정했다.
- 로컬 검증으로 deployment report를 실제 생성/업로드했고 Notion page URL은 `https://www.notion.so/20260307-110654-Parrotkit-Production-Deployment-Report-31cfdc54bb72810681c3e097f42f8343` 이다.
- 주의: 해당 workflow는 GitHub default branch에 있어야 Vercel repository_dispatch가 실제로 트리거된다. 현재 origin default branch는 `main`이다.
